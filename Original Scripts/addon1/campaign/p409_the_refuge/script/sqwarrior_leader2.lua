...
State
{
	...
	OnFigureRespawnEvent
	{
		...
		Conditions = 
		{
			OR
			{
				MapFlagIsTrue	{Name = "mf_SQWarrior_SpawnLeader2Now"},
	...
	OnEvent
	{
		...
		Actions = 
		{
			MapFlagSetFalse	{Name = "mf_SQWarrior_SpawnLeader2Now"},
...
State	
{
	...
	OnEvent
	{
		Conditions = 
		{
			EntityTimerIsElapsed	{Name = "et_DespawnMe", Seconds = 2.2},
		},
		Actions = 
		{
			ObjectSpawnToEntity	{ObjectId = 1433, TargetTag = GetScriptTag(), Direction = 0, Tag = GetScriptTag() .. "_Reward"},
			--ObjectLootItemGenericAdd	{Tag = GetScriptTag() .. "_Reward", DropChance = 100, Level = 26, Skill = 0},
