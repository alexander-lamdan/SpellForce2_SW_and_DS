---------------------------------------------------------------------------------
----
----		Cutscene bei Missionsbeginn
----
---------------------------------------------------------------------------------
--
--
--State
--{
--	StateName = "Init",
--	OnOneTimeEvent
--	{
--		GotoState = "Cutscene5",
--		Conditions =
--		{
--			MapFlagIsTrue {Name = "mf_P301_NightsongFree"},
--			MapFlagIsTrue {Name = "mf_P301_Tutorial"},
--		},
--		Actions =
--		{
--			
--		},
--	},
--}
--
--
--
--State
--{
--	StateName = "Cutscene5",
--	
--	OnOneTimeEvent
--	{
--		Conditions =
--		{  
--		},
--		Actions = 
--		{
--			CutsceneBegin {},
--			CameraSet {X = 568.64, Y = 61.67, Z = 417.23, LookAtX = 567.71, LookAtY = 61.23, LookAtZ = 417.11},
--			EntityTimerStart {Name = "et_Cutscene5"},
--		},
--		
--	},
--	
--	OnOneTimeEvent
--	{
--			Conditions =
--			{
--				EntityTimerIsElapsed {Name = "et_Cutscene5", Seconds = 4},
--			},
--			Actions =
--			{
--				CutsceneSay	{TextTag = "NightSong1", Tag = "NightSong"},
--			},
--	},
--	OnOneTimeEvent
--	{
--			Conditions =
--			{
--				EntityTimerIsElapsed {Name = "et_Cutscene5", Seconds = 10},
--			},
--			Actions =
--			{
--				CutsceneSay	{TextTag = "Avatar3", Tag = "pl_HumanAvatar"},
--			},
--	},
--	OnOneTimeEvent
--	{
--			Conditions =
--			{
--				EntityTimerIsElapsed {Name = "et_Cutscene5", Seconds = 16},
--			},
--			Actions =
--			{
--				CutsceneSay	{TextTag = "NightSong2", Tag = "NightSong"},
--			},
--	},
--	OnOneTimeEvent
--	{
--			Conditions =
--			{
--				EntityTimerIsElapsed {Name = "et_Cutscene5", Seconds = 20},
--			},
--			Actions =
--			{
--				CutsceneSay	{TextTag = "Avatar4", Tag = "pl_HumanAvatar"},
--			},
--	},
--	OnOneTimeEvent
--	{
--			Conditions =
--			{
--				EntityTimerIsElapsed {Name = "et_Cutscene5", Seconds = 24},
--			},
--			Actions =
--			{
--				CutsceneSay	{TextTag = "Avatar5", Tag = "pl_HumanAvatar"},
--			},
--	},
--	OnOneTimeEvent
--	{
--			Conditions =
--			{
--				EntityTimerIsElapsed {Name = "et_Cutscene5", Seconds = 29},
--			},
--			Actions =
--			{
--				CutsceneSay	{TextTag = "NightSong3", Tag = "NightSong"},
--			},
--	},
--	OnOneTimeEvent
--	{
--			Conditions =
--			{
--				EntityTimerIsElapsed {Name = "et_Cutscene5", Seconds = 35},
--			},
--			Actions =
--			{
--				CameraStop {},				
--				CutsceneEnd {},
--				QuestSetSolved {Quest = "SaveNightsong"},
--				QuestSetActive {Quest = "IgniteAlarmFire"},
--				MapFlagSetTrue {Name = "mf_P301_NightsongFreeShown"},
--				--FigurePlayerTransfer {Tag = "NightSong"},
--				
--				FigureVanish {Tag = "NightSong"},
--				PlayerHeroAdd {Player = "pl_Human", HeroId = 25},
--				FigureHeroSpawn {Player = "default", Tag = "Nightsong", X = 293.766, Y = 503.342, Direction = 90},
--			},
--	},
--};
--
--	
